...
import Modules.reportphraser as NFS_RP
...
PHRASE_DBSEARCH_RESULT = {
    ...
    "결과없음-현장프로필": "{nickname} 디엔에이형을 현재까지 수록된 \"디엔에이신원확인정보 데이터베이스\"에서 검색한 결과 일치 건(범죄 현장 등, 구속피의자 등 및 수형인 등) 없음.\r\n",
    "과거건일치-피의자": "{nickname}의 디엔에이형을 현재까지 수록된 \"디엔에이신원확인정보 데이터베이스\"에서 검색한 결과, 다음 [표]의 사건에서 확보된 디엔에이형과 일치함.\r\n",
    "과거건일치-현장프로필": "{nickname} 디엔에이형을 현재까지 수록된 \"디엔에이신원확인정보 데이터베이스\"에서 검색한 결과, 다음 [표]의 사건에서 확보된 디엔에이형과 일치하고, 구속피의자 등 및 수형인 등과 일치 건 없음.\r\n",
    "수형인일치": "{nickname} 디엔에이형을 현재까지 수록된 \"디엔에이신원확인정보 데이터베이스\"에서 검색한 결과, 수형인 등과 일치 건 있음.\r\n",
    "구속피의자일치" :"{nickname} 디엔에이형을 현재까지 수록된 \"디엔에이신원확인정보 데이터베이스\"에서 검색한 결과, 구속피의자 식별코드 \"{code_arrestee}\"의 디엔에이형과 일치함.\r\n"
}
...
RETURN_STATUSES = [
        ...
        ReturnStatus(
            ...
            mixed_phrase="{}는 대전과학수사연구소 독성화학과로 반환했고",
            needs_evidence_num=True,
            process_order=2,
        ),
        ReturnStatus(
            key="폐기",
            single_phrase="감정물은 폐기함.\r\n",
            mixed_phrase="{} 감정물은 폐기하고",
            needs_evidence_num=True,
            process_order=3,
        ),
        ReturnStatus(
...
ETC_CONDITIONS: list[EtcCondition] = [
    ...
    EtcCondition(
        ...
        key="DB저장2",
        phrase=(
            "\"디엔에이신원확인정보 데이터베이스\"의 무결성 확보를 위해 사건과 관련이 없는 디엔에이형(오인신고, 사건과 무관, 수사종결, 불입건 등)은 삭제 요청하기 바람.\r\n"
        ),
        condition=lambda text: "일치 건(범죄 현장 등, 구속피의자 등 및 수형인 등)" in text \
                                or "다음 [표]의 사건에서 확보된 디엔에이형과 일치하고, 구속피의자 등 및 수형인 등과 일치 건 없음." in text 
    ), 
    ...
    EtcCondition(
        key="YSTR일치",
        phrase=" 남성 특이적인 Y-STR 디엔에이형은 STR 디엔에이형과는 달리 남성 세포가 여성 세포에 비해 미량일 때에도 검출될 수 있으며, Y-STR 디엔에이형이 일치할 경우 동일부계 남성이 배제되지 않음.\r\n",
